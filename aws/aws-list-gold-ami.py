...
AMI_ALIAS = dict(  # this should work with describe_images(image_ids=[])
    # aws2='/aws/service/marketplace/prod-rhfhj7ctjlrvy/2.0.20211201.0',
...
def desc_images(productcode, region):
    ...
    :param: region Config object which defines the region to query
    ...
    else:  # AWS Linux2 doesn't have productcode so use session manager to get public AMI
...
def main():
    ...
    if args.REGION:
        ...
        if not valid_region(args.REGION):
            ...
            regions_print(incr=5)
            return 1  # create a Config object defining region to use with a boto3 client
        ...
        avz_list = []  # store region's availability zones
        ...
        if args.template:
            ...
            output = template.render(images=gold_ami, region=args.REGION, avzs=avz_list)
